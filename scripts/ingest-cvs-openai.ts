...
async function main() {
  console.log('🚀 Starting CV ingestion to Qdrant (OpenAI)...\n');
  ...
  const collectionName = process.env.QDRANT_OPENAI_COLLECTION || 'openai-search';

  // Detect embedding dimension by generating a test embedding
  console.log('🔍 Detecting embedding dimension...');
  const testEmbedding = await generateEmbedding('test');
  const vectorSize = testEmbedding.length;
  console.log(`✅ OpenAI model produces ${vectorSize} dimensions\n`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      // Small delay to avoid rate limiting OpenAI API
  ...
  console.log(`\n🎉 Ingestion complete!`);
  console.log(`\nYou can now test OpenAI searches with the toggle in the UI`);
}
