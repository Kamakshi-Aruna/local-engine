...
async function main() {
  console.log('🚀 Starting CV ingestion to Qdrant (Gemini)...\n');
  ...
  const collectionName = process.env.QDRANT_GEMINI_COLLECTION || 'gemini-search';

  // Detect embedding dimension by generating a test embedding
  console.log('🔍 Detecting embedding dimension...');
  const testEmbedding = await generateEmbedding('test');
  const vectorSize = testEmbedding.length;
  console.log(`✅ Gemini model produces ${vectorSize} dimensions\n`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      // Small delay to avoid rate limiting Gemini API
  ...
  console.log(`\n🎉 Ingestion complete!`);
  console.log(`\nYou can now test Gemini searches with the toggle in the UI`);
}
